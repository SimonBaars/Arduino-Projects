...
void MatrixKlok(){
  ...
  if (MatrixCurrentMillis - MatrixPreviousMillis > IntervalEenSec) {
    ...
    if (MatrixNummer >= 0 && MatrixNummer <= 12){
      ...
      Serial.print(MatrixNummer);
    }
    else { 
      MatrixNummer = 0; 
      Serial.print("Matrix reset");
    }
  }
}
...
void Een(){
  byte een[] = {
    0x3f, 0x3f, 0x0c, 0x0c , 0x0c , 0x0f , 0x0e, 0x0c           };
  Wire.beginTransmission(Matrix_Verkeerslicht); 
  Wire.write(0x00);
  Wire.write(een[0]);
  Wire.write(0x00);
...
void Drie(){
  byte drie[] = {
    0x2a, 0x2a, 0x30, 0x2a , 0x2a , 0x30 , 0x2a, 0x2a            };
  Wire.beginTransmission(Matrix_Verkeerslicht); 
  Wire.write(0x00);
  Wire.write(drie[0]);
  Wire.write(0x00);
...
void Vier(){
  byte vier[] = {
    0x03, 0x03, 0x03, 0x63, 0x63,0x33, 0x33, 0x33            };
  Wire.beginTransmission(Matrix_Verkeerslicht); 
  Wire.write(0x00);
  Wire.write(vier[0]);
  Wire.write(0x00);
